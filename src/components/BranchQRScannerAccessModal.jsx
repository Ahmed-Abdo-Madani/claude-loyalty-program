...
function BranchQRScannerAccessModal({ branch, isOpen, onClose, onSuccess }) {
    ...
    const [scannerAccessEnabled, setScannerAccessEnabled] = useState(branch?.scanner_access_enabled ?? true)
    const [showScannerDisableConfirm, setShowScannerDisableConfirm] = useState(false)
    ...
    useEffect(() => {
        if (isOpen && branch) {
            setScannerAccessEnabled(branch.scanner_access_enabled ?? true)
        }
    }, [isOpen, branch?.public_id, branch?.scanner_access_enabled])

    // Generate QR code when modal opens, branch changes, or scanner access is enabled
    useEffect(() => {
        const generateScannerLoginQR = async () => {
            if (!branch?.public_id) return

            try {
                setManagerQrLoading(true)
                const result = await QRCodeGenerator.generateScannerLoginQRCode(
                    branch.public_id,
                    { size: 120 }
                )
                ...
                if (result.success) {
                    ...
                    console.error('Failed to generate scanner login QR:', result.error)
                    setManagerQrData(null)
                }
            } catch (error) {
                console.error('Error generating scanner login QR:', error)
                setManagerQrData(null)
            } finally {
        ...
        if (isOpen && branch?.public_id && scannerAccessEnabled) {
            generateScannerLoginQR()
        }
    }, [isOpen, branch?.public_id, scannerAccessEnabled])
    ...
    // Handle Scanner Access Toggle
    const handleToggleScannerAccess = async (enabled) => {
        if (!branch?.public_id) {
            setScannerAccessEnabled(enabled)
            return
        }

        if (!enabled) {
            setShowScannerDisableConfirm(true)
            return
        }

        await executeScannerToggle(true)
    }

    const executeScannerToggle = async (enabled) => {
        setScannerAccessEnabled(enabled)
        setToggleSaving(true)
        setShowScannerDisableConfirm(false)
        ...
        try {
            const response = await secureApiRequest(`${endpoints.myBranches}/${branch.public_id}`, {
                ...
                body: JSON.stringify({
                    scanner_access_enabled: enabled
                })
            })
            ...
            if (!data.success) {
                throw new Error(data.message || 'Failed to update scanner access')
            }
            // Success - optimistic update holds
        } catch (error) {
            console.error('Error updating scanner access:', error)
            setScannerAccessEnabled(!enabled) // Revert
            setPinSaveError(error.message || 'Failed to update scanner access setting')
        } finally {
    ...
    const handleCopyLoginLink = async () => {
        ...
        try {
            const loginUrl = `${import.meta.env.VITE_BASE_URL || 'https://app.madna.me'}/branch-manager-login?branch=${branch.public_id}&redirect=scanner`
    ...
    return createPortal(
        ...
        >
                ...
                <div className="flex-shrink-0 flex items-center justify-between p-4 sm:p-6 border-b border-gray-200 dark:border-gray-700">
                    <div>
                        <h3 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white flex items-center">
                            🔍 {t('branches.qrScanAccess')}
                        </h3>
                        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                            {branch?.name || 'Branch'} - {t('branches.qrScanAccessDesc')}
                        </p>
                    </div>
                    <button
                        onClick={handleClose}
                            ...
                            <div className="flex items-center justify-between mb-4">
                                <div>
                                    <h3 className="text-sm font-bold text-purple-900 dark:text-purple-200 flex items-center">
                                        🔐 {t('branches.enableQrScanAccess')}
                                    </h3>
                                    <p className="text-xs text-purple-700 dark:text-purple-300 mt-1">
                                        {t('branches.qrScanAccessDesc')}
                                    </p>
                                </div>
                                <input
                                    type="checkbox"
                                    id="scanner_access_enabled"
                                    checked={scannerAccessEnabled}
                                    onChange={(e) => handleToggleScannerAccess(e.target.checked)}
                                        ...
                                        <div className="flex gap-2">
                                            <div className="relative flex-1">
                                                <input
                                                    ...
                                                    placeholder={t('branches.qrScannerPinPlaceholder')}
                                                />
                                                <button
                                                    type="button"
                                        ...
                                        <div className="bg-white dark:bg-gray-800 p-4 rounded-lg border border-purple-200 dark:border-purple-700">
                                            ...
                                            <div className="flex items-center gap-4">
                                                <div className="bg-white p-2 rounded-lg border border-gray-200">
                                                    {managerQrLoading ? (
                                                        ...
                                                        <img
                                                            ...
                                                            alt={t('branches.qrScannerLoginQRAlt')}
                                                ...
                                                <div className="flex-1">
                                                    <p className="text-xs text-purple-700 dark:text-purple-300 mb-2">
                                                        {t('branches.qrScannerLoginQRDesc')}
                                                    </p>
                                            ...
                                            <div className="mt-4 pt-4 border-t border-purple-100 dark:border-purple-900/50">
                                                <button
                                                    ...
                                                    aria-label={t('branches.shareQrScannerLoginLink')}
                                                ...
                                                >
                                                    {linkCopied ? (
                                                        ...
                                                        <>
                                                            <span>🔗</span>
                                                            <span>{t('branches.shareQrScannerLoginLink')}</span>
                                                        </>
                                                    )}
                                                </button>
                                                {copyError && (
                                                ...
                                                <p className="text-[10px] text-gray-400 dark:text-gray-500 mt-2 text-center">
                                                    {t('branches.shareQrScannerLoginLinkDesc')}
                                                </p>
                                            </div>
                                        </div>
                                    )}
            ...
            {/* Confirmation Modal for Disabling Scanner Access */}
            {showScannerDisableConfirm && (
                <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4">
                    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm" onClick={() => setShowScannerDisableConfirm(false)}></div>
                    <div className="relative bg-white dark:bg-gray-800 rounded-xl shadow-2xl p-6 max-w-sm w-full border border-red-200 dark:border-red-800 animation-scale-in">
                        <div className="text-center">
                            ...
                            <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-2">
                                {t('branches.disableQrScannerAccessTitle')}
                            </h3>
                            <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">
                                {t('branches.disableQrScannerAccessMessage')}
                            </p>
                            <div className="flex gap-3">
                                <button
                                    onClick={() => setShowScannerDisableConfirm(false)}
                                    className="flex-1 px-4 py-2 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-lg font-medium hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors"
                                >
                                    {t('common.cancel') || 'Cancel'}
                                </button>
                                <button
                                    onClick={() => executeScannerToggle(false)}
...
export default BranchQRScannerAccessModal
